...
public class MinimalQueue implements Printable {
    ...
    public Object dequeue() { // removes the first element from the queue - the smallest one
        if (isEmpty()) {
            return null;
        } else {
            Node temp = head;
    ...
    public Object remove(double key) { // removes the first of given key
        if (isEmpty()) { // if there are no elements
            ...
            if (head.key == key) { // if the only element is the correct element
                Node temp = head;
                head = null;
                size -= 1;
                return temp.data;
            ...
            if (head.key != key) { // if the element is not the head
                Node current = head.next;
                Node previous = head;
                while (current.key != key) {
                    if (current.next == null) { // if event the last isn't it
                        return null;
                    } else {
                        previous = current;
                        current = current.next;
    ...
    @Override public String toString() {
        if (!isEmpty()) {
            ...
            return ("MinimalQueue size: "+size+"; MinimalQueue contents: ["+printedNodes+"]");
        } else {
            return ("MinimalQueue is empty");
        }
    }
